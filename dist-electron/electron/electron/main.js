...
function createWindow() {
    const win = new electron_1.BrowserWindow({
        ...
        icon: electron_1.app.isPackaged
            ? path_1.default.join(__dirname, '../build/icons/icon.ico')
            : path_1.default.join(process.cwd(), 'build/icons/icon.ico'),
        webPreferences: {
 ...
 */
function processVideoWithFfmpeg(options) {
    const resolvedFfmpegPath = (ffmpeg_static_1.default || '').toString();
    ...
    const baseOutputDir = options.outputDir ||
        path_1.default.join(electron_1.app.getPath('videos'), 'PixelMonkey-Frames', Date.now().toString());
    ensureDirectory(baseOutputDir);
    const outputPattern = path_1.default.join(baseOutputDir, 'frame_%05d.png');
    ...
    // 如果有文件数据，先写入临时文件
    const processVideoFile = async (videoPath) => {
        const args = [
            '-i',
            videoPath,
            '-vf',
        ...
        return new Promise((resolve) => {
            const child = (0, child_process_1.spawn)(resolvedFfmpegPath, args, {
                windowsHide: true,
            });
            let stderr = '';
            ...
            child.on('close', (code) => {
                ...
                let frameCount = 0;
                try {
                    const files = fs_1.default.readdirSync(baseOutputDir);
    ...
    if (options.fileData && options.fileData.length > 0) {
        const tempVideoPath = path_1.default.join(baseOutputDir, options.inputPath);
        try {
            fs_1.default.writeFileSync(tempVideoPath, new Uint8Array(options.fileData));
...
electron_1.app.whenReady().then(() => {
    createWindow();
    electron_1.ipcMain.handle('video:process', async (_event, options) => {
        return processVideoWithFfmpeg(options);
    });
    // 预览单个视频帧
    electron_1.ipcMain.handle('video:previewFrame', async (_event, options) => {
        const resolvedFfmpegPath = (ffmpeg_static_1.default || '').toString();
        if (!resolvedFfmpegPath) {
            return {
                success: false,
        ...
        try {
            ...
            // 临时文件路径
            const tempOutput = path_1.default.join(os_1.default.tmpdir(), `preview_frame_${Date.now()}.png`);
            // 使用 ffmpeg 提取指定帧
            const args = [
                '-i', options.inputPath,
            ...
            await new Promise((resolve, reject) => {
                const child = (0, child_process_1.spawn)(resolvedFfmpegPath, args, { windowsHide: true });
                let stderr = '';
            ...
            // 读取临时文件并转换为 base64
            const frameBuffer = fs_1.default.readFileSync(tempOutput);
    ...
    // 获取视频信息
    electron_1.ipcMain.handle('video:getInfo', async (_event, inputPath) => {
    ...
    ;
    // 导出视频序列帧
    electron_1.ipcMain.handle('video:exportSequence', async (_event, options) => {
        return processVideoWithFfmpeg(options);
    });
